...
%%	<code>{values, V}</code> where <code>V</code> is a list
...
t(Time_This, Continue, AccFn, Acc0, PostFn) ->
  if is_function(Time_This, 0) -> ok;
     true -> error({"time_it:t: Time_This should be a function of no arguments", {time_this, Time_This}})
  end,
