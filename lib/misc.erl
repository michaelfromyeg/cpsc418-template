...
%%      </li>
%%    </ul>
%%    <p>If <code>N <it>=0</it></code> and <code>M <it>&#60; 0</it></code>,
...
%%    produces a floating point overflow.</p>
%%    <p>Finally, if <code>N</code> is a (deep) list and <code>M</code> is a number,
%%    <code>pow(N, M)</code> returns the list obtained by applying <code>pow</code>
%%    to each element of <code>N</code> using exponent <code>M</code>.  Likewise if
%%    <code>N</code> is a number and <code>M</code> is a (deep) list.  If <code>N</code>
%%    and <code>M</code> are both lists of the same length, the <code>pow</code> is
%%    applied to the pairs of corresponding values of <code>N</code> and <code>M</code>.
%%    </p>
pow(N, M) when is_integer(N), is_integer(M), (M >= 0) ->
  rp_pow(N, M, 1);
pow(0, M) when (M < 0) -> error(badarith);
...
pow(N, M) when is_number(N), is_number(M) -> math:pow(N,M);
...
rlist2(N,  M, Rlist) -> rlist2(N-1, M, [rand:uniform(M) | Rlist]).
rlist2f(0, _A, Rlist) -> Rlist;
rlist2f(N,  A, Rlist) -> rlist2f(N-1, A, [A*rand:uniform() | Rlist]).
...
msg_dump_messages(MyPid) ->
  receive
    Msg ->
      io:format("~w:   ~100P~n", [MyPid, Msg, 12]),
      msg_dump_messages(MyPid)
    after 0 ->
...
%%   <li><code>N</code> must be an integer or a list.</li>
%%     <ul>
%%       <li> If <code>is_list(L)</code> then <code>intervals(Lo, Hi, L)</code>
%%            is equivalent to <code>intervals(Lo, Hi, length(L)</code>.
%%            This feature was added on 2017 Feb. 2 for consistency with
%%            {@link cut}.
%%       </li>
%%       <li> The rest of this description of <code>intervals</code> describes
%%            <code>interavals(Lo, Hi, N) when is_integer(N)</code>.
%%       </li>
%%     </ul>
...
intervals(Lo, Hi, N) when is_integer(N) and is_number(Lo) and is_number(Hi)
  ...
  Next = Lo + ((Hi - Lo) / N),
  [{Lo, Next} | intervals(Next, Hi, N-1)];
intervals(Lo, Hi, L) when is_list(L) and is_number(Lo) and is_number(Hi) ->
  intervals(Lo, Hi, length(L)).
